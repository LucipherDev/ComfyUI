...
class PairConditioningSetProperties:
    ...
    def set_properties(self, positive_NEW, negative_NEW,
                       ...
                       mask: torch.Tensor=None, hooks: totoro.hooks.HookGroup=None, timesteps: tuple=None):
        final_positive, final_negative = totoro.hooks.set_conds_props(conds=[positive_NEW, negative_NEW],
...
class PairConditioningSetPropertiesAndCombine:
    ...
    def set_properties(self, positive, negative, positive_NEW, negative_NEW,
                       strength: float, set_cond_area: str,
                       mask: torch.Tensor=None, hooks: totoro.hooks.HookGroup=None, timesteps: tuple=None):
        final_positive, final_negative = totoro.hooks.set_conds_props_and_combine(conds=[positive, negative], new_conds=[positive_NEW, negative_NEW],
                                                                                strength=strength, set_cond_area=set_cond_area,
                                                                                mask=mask, hooks=hooks, timesteps_range=timesteps)
        return (final_positive, final_negative)
...
class ConditioningSetProperties:
    ...
    def set_properties(self, cond_NEW,
                       strength: float, set_cond_area: str,
                       mask: torch.Tensor=None, hooks: totoro.hooks.HookGroup=None, timesteps: tuple=None):
        (final_cond,) = totoro.hooks.set_conds_props(conds=[cond_NEW],
                                                   strength=strength, set_cond_area=set_cond_area,
                                                   mask=mask, hooks=hooks, timesteps_range=timesteps)
        return (final_cond,)
...
class ConditioningSetPropertiesAndCombine:
    ...
    def set_properties(self, cond, cond_NEW,
                       strength: float, set_cond_area: str,
                       mask: torch.Tensor=None, hooks: totoro.hooks.HookGroup=None, timesteps: tuple=None):
        (final_cond,) = totoro.hooks.set_conds_props_and_combine(conds=[cond], new_conds=[cond_NEW],
                                                               strength=strength, set_cond_area=set_cond_area,
                                                               mask=mask, hooks=hooks, timesteps_range=timesteps)
        return (final_cond,)
...
class PairConditioningCombine:
    ...
    def combine(self, positive_A, negative_A, positive_B, negative_B):
        final_positive, final_negative = totoro.hooks.set_conds_props_and_combine(conds=[positive_A, negative_A], new_conds=[positive_B, negative_B],)
        return (final_positive, final_negative,)
...
class PairConditioningSetDefaultAndCombine:
    ...
    def set_default_and_combine(self, positive, negative, positive_DEFAULT, negative_DEFAULT,
                                hooks: totoro.hooks.HookGroup=None):
        final_positive, final_negative = totoro.hooks.set_default_conds_and_combine(conds=[positive, negative], new_conds=[positive_DEFAULT, negative_DEFAULT],
                                                                                   hooks=hooks)
        return (final_positive, final_negative)
...
class ConditioningSetDefaultAndCombine:
    ...
    def set_default_and_combine(self, cond, cond_DEFAULT,
                                hooks: totoro.hooks.HookGroup=None):
        (final_conditioning,) = totoro.hooks.set_default_conds_and_combine(conds=[cond], new_conds=[cond_DEFAULT],
                                                                        hooks=hooks)
...
class SetClipHooks:
    ...
    def apply_hooks(self, clip: 'CLIP', schedule_clip: bool, apply_to_conds: bool, hooks: totoro.hooks.HookGroup=None):
        if hooks is not None:
            ...
            clip.patcher.register_all_hook_patches(hooks.get_dict_repr(), totoro.hooks.EnumWeightTarget.Clip)
        return (clip,)
...
class CreateHookLora:
    ...
    def create_hook(self, lora_name: str, strength_model: float, strength_clip: float, prev_hooks: totoro.hooks.HookGroup=None):
        if prev_hooks is None:
            prev_hooks = totoro.hooks.HookGroup()
        prev_hooks.clone()
        ...
        hooks = totoro.hooks.create_hook_lora(lora=lora, strength_model=strength_model, strength_clip=strength_clip)
...
class CreateHookLoraModelOnly(CreateHookLora):
    ...
    def create_hook_model_only(self, lora_name: str, strength_model: float, prev_hooks: totoro.hooks.HookGroup=None):
...
class CreateHookModelAsLora:
    ...
    def create_hook(self, ckpt_name: str, strength_model: float, strength_clip: float,
                    prev_hooks: totoro.hooks.HookGroup=None):
        if prev_hooks is None:
            prev_hooks = totoro.hooks.HookGroup()
        prev_hooks.clone()
        ...
        if weights_model is None:
            ...
            weights_model = totoro.hooks.get_patch_weights_from_model(out[0])
            weights_clip = totoro.hooks.get_patch_weights_from_model(out[1].patcher if out[1] else out[1])
            self.loaded_weights = (ckpt_path, weights_model, weights_clip)

        hooks = totoro.hooks.create_hook_model_as_lora(weights_model=weights_model, weights_clip=weights_clip,
...
class SetHookKeyframes:
    ...
    def set_hook_keyframes(self, hooks: totoro.hooks.HookGroup, hook_kf: totoro.hooks.HookKeyframeGroup=None):
...
class CreateHookKeyframe:
    ...
    def create_hook_keyframe(self, strength_mult: float, start_percent: float, prev_hook_kf: totoro.hooks.HookKeyframeGroup=None):
        if prev_hook_kf is None:
            prev_hook_kf = totoro.hooks.HookKeyframeGroup()
        prev_hook_kf = prev_hook_kf.clone()
        keyframe = totoro.hooks.HookKeyframe(strength=strength_mult, start_percent=start_percent)
...
class CreateHookKeyframesInterpolated:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                ...
                "interpolation": (totoro.hooks.InterpolationMethod._LIST, ),
                "start_percent": ("FLOAT", {"default": 0.0, "min": 0.0, "max": 1.0, "step": 0.001}),
                "end_percent": ("FLOAT", {"default": 1.0, "min": 0.0, "max": 1.0, "step": 0.001}),
    ...
    def create_hook_keyframes(self, strength_start: float, strength_end: float, interpolation: str,
                              ...
                              print_keyframes=False, prev_hook_kf: totoro.hooks.HookKeyframeGroup=None):
        if prev_hook_kf is None:
            prev_hook_kf = totoro.hooks.HookKeyframeGroup()
        prev_hook_kf = prev_hook_kf.clone()
        percents = totoro.hooks.InterpolationMethod.get_weights(num_from=start_percent, num_to=end_percent, length=keyframes_count,
                                                               method=totoro.hooks.InterpolationMethod.LINEAR)
        strengths = totoro.hooks.InterpolationMethod.get_weights(num_from=strength_start, num_to=strength_end, length=keyframes_count, method=interpolation)
        ...
        for percent, strength in zip(percents, strengths):
            ...
            if is_first:
                guarantee_steps = 1
                is_first = False
            prev_hook_kf.add(totoro.hooks.HookKeyframe(strength=strength, start_percent=percent, guarantee_steps=guarantee_steps))
            if print_keyframes:
...
class CreateHookKeyframesFromFloats:
    ...
    def create_hook_keyframes(self, floats_strength: Union[float, list[float]],
                              ...
                              prev_hook_kf: totoro.hooks.HookKeyframeGroup=None, print_keyframes=False):
        if prev_hook_kf is None:
            prev_hook_kf = totoro.hooks.HookKeyframeGroup()
        prev_hook_kf = prev_hook_kf.clone()
        ...
        percents = totoro.hooks.InterpolationMethod.get_weights(num_from=start_percent, num_to=end_percent, length=len(floats_strength),
                                                               method=totoro.hooks.InterpolationMethod.LINEAR)
...
class SetModelHooksOnCond:
    ...
    def attach_hook(self, conditioning, hooks: totoro.hooks.HookGroup):
        return (totoro.hooks.set_hooks_for_conditioning(conditioning, hooks),)
...
class CombineHooks:
    ...
    def combine_hooks(self,
                      hooks_A: totoro.hooks.HookGroup=None,
                      hooks_B: totoro.hooks.HookGroup=None):
        candidates = [hooks_A, hooks_B]
        return (totoro.hooks.HookGroup.combine_all_hooks(candidates),)
...
class CombineHooksFour:
    ...
    def combine_hooks(self,
                      hooks_A: totoro.hooks.HookGroup=None,
                      hooks_B: totoro.hooks.HookGroup=None,
                      hooks_C: totoro.hooks.HookGroup=None,
                      hooks_D: totoro.hooks.HookGroup=None):
...
class CombineHooksEight:
    ...
    def combine_hooks(self,
                      ...
                      hooks_C: totoro.hooks.HookGroup=None,
                      hooks_D: totoro.hooks.HookGroup=None,
                      hooks_E: totoro.hooks.HookGroup=None,
                      hooks_F: totoro.hooks.HookGroup=None,
                      hooks_G: totoro.hooks.HookGroup=None,
                      hooks_H: totoro.hooks.HookGroup=None):
