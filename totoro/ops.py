...
cast_to = totoro.model_management.cast_to #TODO: remove once no more references

def cast_to_input(weight, input, non_blocking=False, copy=True):
    return totoro.model_management.cast_to(weight, input.dtype, input.device, non_blocking=non_blocking, copy=copy)
...
def cast_bias_weight(s, input=None, dtype=None, device=None, bias_dtype=None):
    ...
    if s.bias is not None:
        ...
        bias = totoro.model_management.cast_to(s.bias, bias_dtype, device, non_blocking=non_blocking, copy=has_function)
        if has_function:
            bias = s.bias_function(bias)

    has_function = s.weight_function is not None
    weight = totoro.model_management.cast_to(s.weight, dtype, device, non_blocking=non_blocking, copy=has_function)
    if has_function:
    ...
    class scaled_fp8_op(manual_cast):
        class Linear(manual_cast.Linear):
            ...
            def set_weight(self, weight, inplace_update=False, seed=None, **kwargs):
                weight = totoro.float.stochastic_rounding(weight / self.scale_weight.to(device=weight.device, dtype=weight.dtype), self.weight.dtype, seed=seed)
                if inplace_update:
...
def pick_operations(weight_dtype, compute_dtype, load_device=None, disable_fast_fp8=False, fp8_optimizations=False, scaled_fp8=None):
    fp8_compute = totoro.model_management.supports_fp8_compute(load_device)
