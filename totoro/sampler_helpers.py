...
def prepare_mask(noise_mask, shape, device):
    return totoro.utils.reshape_mask(noise_mask, shape).to(device)
...
def get_hooks_from_cond(cond, hooks_dict: dict[totoro.hooks.EnumHookType, dict[totoro.hooks.Hook, None]]):
    # get hooks from conds, and collect cnets so they can be checked for extra_hooks
    cnets: list[ControlBase] = []
    for c in cond:
        if 'hooks' in c:
            for hook in c['hooks'].hooks:
                hook: totoro.hooks.Hook
    ...
    extra_hooks = totoro.hooks.HookGroup.combine_all_hooks(hooks_list)
...
def get_additional_models(conds, dtype):
    ...
    gligen = []
    add_models = []
    hooks: dict[totoro.hooks.EnumHookType, dict[totoro.hooks.Hook, None]] = {}
    ...
    hook_models = [x.model for x in hooks.get(totoro.hooks.EnumHookType.AddModels, {}).keys()]
...
def prepare_model_patcher(model: 'ModelPatcher', conds, model_options: dict):
    ...
    model_options["transformer_options"]["wrappers"] = totoro.patcher_extension.copy_nested_dicts(model.wrappers)
    model_options["transformer_options"]["callbacks"] = totoro.patcher_extension.copy_nested_dicts(model.callbacks)
    # register hooks on model/model_options
    model.register_all_hook_patches(hooks, totoro.hooks.EnumWeightTarget.Model, model_options)
