...
import totoro.ldm.genmo.joint_model.asymm_models_joint
...
import totoro.ldm.lightricks.model
import totoro.ldm.hunyuan_video.model
...
class BaseModel(torch.nn.Module):
    def __init__(self, model_config, model_type=ModelType.EPS, device=None, unet_model=UNetModel):
        ...
        if not unet_config.get("disable_unet_model_creation", False):
            if model_config.custom_operations is None:
                ...
                operations = totoro.ops.pick_operations(unet_config.get("dtype", None), self.manual_cast_dtype, fp8_optimizations=fp8, scaled_fp8=model_config.scaled_fp8)
            else:
    ...
    def apply_model(self, x, t, c_concat=None, c_crossattn=None, control=None, transformer_options={}, **kwargs):
        return totoro.patcher_extension.WrapperExecutor.new_class_executor(
            self._apply_model,
            self,
            totoro.patcher_extension.get_all_wrappers(totoro.patcher_extension.WrappersMP.APPLY_MODEL, transformer_options)
    ...
    def extra_conds(self, **kwargs):
        ...
        if concat_cond is not None:
            out['c_concat'] = totoro.conds.CONDNoiseShape(concat_cond)
...
class Flux(BaseModel):
    ...
    def extra_conds(self, **kwargs):
        ...
        if attention_mask is not None:
            ...
            out['attention_mask'] = totoro.conds.CONDRegular(attention_mask)
        out['guidance'] = totoro.conds.CONDRegular(torch.FloatTensor([kwargs.get("guidance", 3.5)]))
        return out

class GenmoMochi(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(model_config, model_type, device=device, unet_model=totoro.ldm.genmo.joint_model.asymm_models_joint.AsymmDiTJoint)
    ...
    def extra_conds(self, **kwargs):
        ...
        if attention_mask is not None:
            out['attention_mask'] = totoro.conds.CONDRegular(attention_mask)
            out['num_tokens'] = totoro.conds.CONDConstant(max(1, torch.sum(attention_mask).item()))
        cross_attn = kwargs.get("cross_attn", None)
        if cross_attn is not None:
            out['c_crossattn'] = totoro.conds.CONDRegular(cross_attn)
        return out
...
class LTXV(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLUX, device=None):
        super().__init__(model_config, model_type, device=device, unet_model=totoro.ldm.lightricks.model.LTXVModel) #TODO
    ...
    def extra_conds(self, **kwargs):
        ...
        if guiding_latent is not None:
            out['guiding_latent'] = totoro.conds.CONDRegular(guiding_latent)

        guiding_latent_noise_scale = kwargs.get("guiding_latent_noise_scale", None)
        if guiding_latent_noise_scale is not None:
            out["guiding_latent_noise_scale"] = totoro.conds.CONDConstant(guiding_latent_noise_scale)

        out['frame_rate'] = totoro.conds.CONDConstant(kwargs.get("frame_rate", 25))
        return out

class HunyuanVideo(BaseModel):
    def __init__(self, model_config, model_type=ModelType.FLOW, device=None):
        super().__init__(model_config, model_type, device=device, unet_model=totoro.ldm.hunyuan_video.model.HunyuanVideo)
    ...
    def extra_conds(self, **kwargs):
        ...
        if cross_attn is not None:
            out['c_crossattn'] = totoro.conds.CONDRegular(cross_attn)
        out['guidance'] = totoro.conds.CONDRegular(torch.FloatTensor([kwargs.get("guidance", 6.0)]))
        return out
