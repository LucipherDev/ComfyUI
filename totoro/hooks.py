...
if TYPE_CHECKING:
    from totoro.model_patcher import ModelPatcher, PatcherInjection
    ...
    def add_hook_patches(self, model: 'ModelPatcher', model_options: dict, target: EnumWeightTarget, registered: list[Hook]):
        ...
        if self.need_weight_init:
            ...
            if target == EnumWeightTarget.Model:
                ...
                key_map = totoro.lora.model_lora_keys_clip(model.model, key_map)
            weights = totoro.lora.load_lora(self.weights, key_map, log_missing=False)
        else:
            if target == EnumWeightTarget.Model:
...
class WrapperHook(Hook):
    ...
    def add_hook_patches(self, model: 'ModelPatcher', model_options: dict, target: EnumWeightTarget, registered: list[Hook]):
        ...
        totoro.patcher_extension.merge_nested_dicts(model_options, add_model_options, copy_dict1=False)
        registered.append(self)
        return True
...
def load_hook_lora_for_models(model: 'ModelPatcher', clip: 'CLIP', lora: dict[str, torch.Tensor],
    ...
    hook = WeightHook()
    hook_group.add(hook)
    loaded: dict[str] = totoro.lora.load_lora(lora, key_map)
    if model is not None:
