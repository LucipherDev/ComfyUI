...
import totoro.lora
import totoro.hooks
import totoro.patcher_extension
from totoro.patcher_extension import CallbacksMP, WrappersMP, PatcherInjection
from totoro.totoro_types import UnetWrapperFunction
...
def create_model_options_clone(orig_model_options: dict):
    return totoro.patcher_extension.copy_nested_dicts(orig_model_options)
...
def wipe_lowvram_weight(m):
    if hasattr(m, "prev_totoro_cast_weights"):
        m.totoro_cast_weights = m.prev_totoro_cast_weights
        del m.prev_totoro_cast_weights
...
class LowVramPatch:
    ...
    def __call__(self, weight):
        ...
        if intermediate_dtype not in [torch.float32, torch.float16, torch.bfloat16]: #intermediate_dtype has to be one that is supported in math ops
            ...
            return totoro.float.stochastic_rounding(totoro.lora.calculate_weight(self.patches[self.key], weight.to(intermediate_dtype), self.key, intermediate_dtype=intermediate_dtype), weight.dtype, seed=string_to_seed(self.key))

        return totoro.lora.calculate_weight(self.patches[self.key], weight, self.key, intermediate_dtype=intermediate_dtype)
...
def get_key_weight(model, key):
    ...
    if len(op_keys) < 2:
        weight = totoro.utils.get_attr(model, key)
    else:
        op = totoro.utils.get_attr(model, op_keys[0])
        try:
...
class ModelPatcher:
    def __init__(self, model, load_device, offload_device, size=0, weight_inplace_update=False):
        ...
        self.hook_patches: dict[totoro.hooks._HookRef] = {}
        self.hook_patches_backup: dict[totoro.hooks._HookRef] = {}
        self.hook_backup: dict[str, tuple[torch.Tensor, torch.device]] = {}
        self.cached_hook_patches: dict[totoro.hooks.HookGroup, dict[str, torch.Tensor]] = {}
        self.current_hooks: Optional[totoro.hooks.HookGroup] = None
        self.forced_hooks: Optional[totoro.hooks.HookGroup] = None  # NOTE: only used for CLIP at this time
        self.is_clip = False
        self.hook_mode = totoro.hooks.EnumHookMode.MaxSpeed
    ...
    def patch_weight_to_device(self, key, device_to=None, inplace_update=False):
        ...
        out_weight = totoro.lora.calculate_weight(self.patches[key], temp_weight, key)
        if set_func is None:
            out_weight = totoro.float.stochastic_rounding(out_weight, weight.dtype, seed=string_to_seed(key))
            if inplace_update:
        ...
        for n, m in self.model.named_modules():
            ...
            for name, param in m.named_parameters(recurse=True):
                if name not in params:
                    ...
                    break
            if not skip and (hasattr(m, "totoro_cast_weights") or len(params) > 0):
                loading.append((totoro.model_management.module_size(m), n, m, params))
        return loading
    ...
    def load(self, device_to=None, lowvram_model_memory=0, force_patch_weights=False, full_load=False):
        with self.use_ejected():
            ...
            for x in loading:
                ...
                if not full_load and hasattr(m, "totoro_cast_weights"):
                    if mem_counter + module_mem >= lowvram_model_memory:
                        lowvram_weight = True
                        lowvram_counter += 1
                        if hasattr(m, "prev_totoro_cast_weights"): #Already lowvramed
                            continue
                ...
                if lowvram_weight:
                    ...
                    m.prev_totoro_cast_weights = m.totoro_cast_weights
                    m.totoro_cast_weights = True
                else:
                    if hasattr(m, "totoro_cast_weights"):
                        if m.totoro_cast_weights:
            ...
            for x in load_completely:
                ...
                m = x[2]
                params = x[3]
                if hasattr(m, "totoro_patched_weights"):
                    if m.totoro_patched_weights == True:
                        continue
        ...
        if unpatch_weights:
            ...
            for k in keys:
                ...
                if bk.inplace_update:
                    totoro.utils.copy_to_param(self.model, k, bk.weight)
                else:
                    totoro.utils.set_attr_param(self.model, k, bk.weight)
    ...
    def partially_unload(self, device_to, memory_to_free=0):
        with self.use_ejected():
            ...
            for unload in unload_list:
                ...
                lowvram_possible = hasattr(m, "totoro_cast_weights")
                if hasattr(m, "totoro_patched_weights") and m.totoro_patched_weights == True:
                    move_weight = True
                    for param in params:
                        key = "{}.{}".format(n, param)
                        ...
                        if bk is not None:
                            ...
                            if bk.inplace_update:
                                totoro.utils.copy_to_param(self.model, key, bk.weight)
                            else:
                                totoro.utils.set_attr_param(self.model, key, bk.weight)
                            self.backup.pop(key)
                    ...
                    if move_weight:
                        ...
                        if lowvram_possible:
                            ...
                            m.prev_totoro_cast_weights = m.totoro_cast_weights
                            m.totoro_cast_weights = True
                        m.totoro_patched_weights = False
    ...
    def calculate_weight(self, patches, weight, key, intermediate_dtype=torch.float32):
        print("WARNING the ModelPatcher.calculate_weight function is deprecated, please use: totoro.lora.calculate_weight instead")
        return totoro.lora.calculate_weight(patches, weight, key, intermediate_dtype=intermediate_dtype)
    ...
    def set_hook_mode(self, hook_mode: totoro.hooks.EnumHookMode):
        self.hook_mode = hook_mode
    
    def prepare_hook_patches_current_keyframe(self, t: torch.Tensor, hook_group: totoro.hooks.HookGroup):
        curr_t = t[0]
    ...
    def register_all_hook_patches(self, hooks_dict: dict[totoro.hooks.EnumHookType, dict[totoro.hooks.Hook, None]], target: totoro.hooks.EnumWeightTarget, model_options: dict=None):
        self.restore_hook_patches()
        registered_hooks: list[totoro.hooks.Hook] = []
        # handle WrapperHooks, if model_options provided
        if model_options is not None:
            for hook in hooks_dict.get(totoro.hooks.EnumHookType.Wrappers, {}):
                hook.add_hook_patches(self, model_options, target, registered_hooks)
        # handle WeightHooks
        weight_hooks_to_register: list[totoro.hooks.WeightHook] = []
        for hook in hooks_dict.get(totoro.hooks.EnumHookType.Weight, {}):
    ...
    def add_hook_patches(self, hook: totoro.hooks.WeightHook, patches, strength_patch=1.0, strength_model=1.0):
        with self.use_ejected():
    ...
    def get_combined_hook_patches(self, hooks: totoro.hooks.HookGroup):
    ...
    def apply_hooks(self, hooks: totoro.hooks.HookGroup, transformer_options: dict=None, force_apply=False):
    ...
    def patch_hooks(self, hooks: totoro.hooks.HookGroup):
        with self.use_ejected():
            self.unpatch_hooks()
            if hooks is not None:
                model_sd_keys = list(self.model_state_dict().keys())
                memory_counter = None
                if self.hook_mode == totoro.hooks.EnumHookMode.MaxSpeed:
                    # TODO: minimum_counter should have a minimum that conforms to loaded model requirements
                    memory_counter = MemoryCounter(initial=totoro.model_management.get_free_memory(self.load_device),
                                                minimum=totoro.model_management.minimum_inference_memory()*2)
    ...
    def patch_cached_hook_weights(self, cached_weights: dict, key: str, memory_counter: MemoryCounter):
        if key not in self.hook_backup:
            weight: torch.Tensor = totoro.utils.get_attr(self.model, key)
        ...
        totoro.utils.copy_to_param(self.model, key, cached_weights[key][0].to(device=cached_weights[key][1]))

    def clear_cached_hook_weights(self):
        self.cached_hook_patches.clear()
        self.patch_hooks(None)

    def patch_hook_weight_to_device(self, hooks: totoro.hooks.HookGroup, combined_patches: dict, key: str, original_weights: dict, memory_counter: MemoryCounter):
        ...
        temp_weight = totoro.model_management.cast_to_device(weight, weight.device, torch.float32, copy=True)
        if convert_func is not None:
            temp_weight = convert_func(temp_weight, inplace=True)

        out_weight = totoro.lora.calculate_weight(combined_patches[key],
                                                 temp_weight,
    ...
    def unpatch_hooks(self) -> None:
        with self.use_ejected():
            ...
            for k in keys:
                totoro.utils.copy_to_param(self.model, k, self.hook_backup[k][0].to(device=self.hook_backup[k][1]))
