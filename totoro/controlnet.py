...
import totoro.ldm.hydit.controlnet
import totoro.ldm.flux.controlnet
import totoro.cldm.dit_embedder
from typing import TYPE_CHECKING
if TYPE_CHECKING:
    from totoro.hooks import HookGroup
        ...
        if self.cond_hint is None or x_noisy.shape[2] * self.compression_ratio != self.cond_hint.shape[2] or x_noisy.shape[3] * self.compression_ratio != self.cond_hint.shape[3]:
            ...
            if len(self.extra_concat_orig) > 0:
                ...
                for c in self.extra_concat_orig:
                    ...
                    c = totoro.utils.common_upscale(c, self.cond_hint.shape[3], self.cond_hint.shape[2], self.upscale_algorithm, "center")
                    to_concat.append(totoro.utils.repeat_to_batch_size(c, self.cond_hint.shape[0]))
...
def controlnet_config(sd, model_options={}):
    model_config = totoro.model_detection.model_config_from_unet(sd, "", True)

    unet_dtype = model_options.get("dtype", None)
    if unet_dtype is None:
        weight_dtype = totoro.utils.weight_dtype(sd)

        supported_inference_dtypes = list(model_config.supported_inference_dtypes)
        if weight_dtype is not None:
            supported_inference_dtypes.append(weight_dtype)

        unet_dtype = totoro.model_management.unet_dtype(model_params=-1, supported_dtypes=supported_inference_dtypes)
    ...
    if operations is None:
        operations = totoro.ops.pick_operations(unet_dtype, manual_cast_dtype, disable_fast_fp8=True)

    offload_device = totoro.model_management.unet_offload_device()
...
def load_controlnet_mmdit(sd, model_options={}):
    ...
    control_model = totoro.cldm.mmdit.ControlNet(num_blocks=num_blocks, control_latent_channels=control_latent_channels, operations=operations, device=offload_device, dtype=unet_dtype, **model_config.unet_config)
...
def load_controlnet_sd35(sd, model_options={}):
    ...
    new_sd = {}
    for k in totoro.utils.MMDIT_MAP_BASIC:
        if k[1] in sd:
    ...
    num_blocks = totoro.model_detection.count_blocks(new_sd, 'transformer_blocks.{}.')

    load_device = totoro.model_management.get_torch_device()
    offload_device = totoro.model_management.unet_offload_device()
    unet_dtype = totoro.model_management.unet_dtype(model_params=-1)
    ...
    control_model = totoro.cldm.dit_embedder.ControlNetEmbedder(img_size=None,
                                                               patch_size=2,
                                                               in_chans=16,
...
def load_controlnet_hunyuandit(controlnet_data, model_options={}):
    ...
    control_model = totoro.ldm.hydit.controlnet.HunYuanControlNet(operations=operations, device=offload_device, dtype=unet_dtype)
    control_model = controlnet_load_state_dict(control_model, controlnet_data)

    latent_format = totoro.latent_formats.SDXL()
...
def load_controlnet_flux_xlabs_mistoline(sd, mistoline=False, model_options={}):
    ...
    control_model = totoro.ldm.flux.controlnet.ControlNetFlux(mistoline=mistoline, operations=operations, device=offload_device, dtype=unet_dtype, **model_config.unet_config)
    control_model = controlnet_load_state_dict(control_model, sd)
...
def load_controlnet_flux_instantx(sd, model_options={}):
    ...
    control_model = totoro.ldm.flux.controlnet.ControlNetFlux(latent_input=True, num_union_modes=num_union_modes, control_latent_channels=control_latent_channels, operations=operations, device=offload_device, dtype=unet_dtype, **model_config.unet_config)
    control_model = controlnet_load_state_dict(control_model, new_sd)

    latent_format = totoro.latent_formats.Flux()
    extra_conds = ['y', 'guidance']
    control = ControlNet(control_model, compression_ratio=1, latent_format=latent_format, concat_mask=concat_mask, load_device=load_device, manual_cast_dtype=manual_cast_dtype, extra_conds=extra_conds)
    return control

def convert_mistoline(sd):
    return totoro.utils.state_dict_prefix_replace(sd, {"single_controlnet_blocks.": "controlnet_single_blocks."})
...
def load_controlnet_state_dict(state_dict, model=None, model_options={}):
    ...
    unet_dtype = model_options.get("dtype", None)
    if unet_dtype is None:
        weight_dtype = totoro.utils.weight_dtype(controlnet_data)

        if supported_inference_dtypes is None:
            supported_inference_dtypes = [totoro.model_management.unet_dtype()]
    ...
    operations = model_options.get("custom_operations", None)
    if operations is None:
        operations = totoro.ops.pick_operations(unet_dtype, manual_cast_dtype)

    controlnet_config["operations"] = operations
    controlnet_config["dtype"] = unet_dtype
    controlnet_config["device"] = totoro.model_management.unet_offload_device()
    controlnet_config.pop("out_channels")
...
def load_controlnet(ckpt_path, model=None, model_options={}):
    ...
    cnet = load_controlnet_state_dict(totoro.utils.load_torch_file(ckpt_path, safe_load=True), model=model, model_options=model_options)
    if cnet is None:
