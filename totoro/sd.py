...
from totoro.utils import ProgressBar
from .ldm.models.autoencoder import AutoencoderKL, AutoencodingEngine
from .ldm.cascade.stage_a import StageA
from .ldm.cascade.stage_c_coder import StageC_coder
from .ldm.audio.autoencoder import AudioOobleckVAE
import totoro.ldm.genmo.vae.model
import totoro.ldm.lightricks.vae.causal_video_autoencoder
import yaml
...
import totoro.text_encoders.long_clipl
import totoro.text_encoders.genmo
import totoro.text_encoders.lt
import totoro.text_encoders.hunyuan_video

import totoro.model_patcher
import totoro.lora
import totoro.lora_convert
import totoro.hooks
import totoro.t2i_adapter.adapter
import totoro.taesd.taesd

import totoro.ldm.flux.redux
...
def load_lora_for_models(model, clip, lora, strength_model, strength_clip):
    ...
    lora = totoro.lora_convert.convert_lora(lora)
...
class CLIP:
    def __init__(self, target=None, embedding_directory=None, no_init=False, tokenizer_data={}, parameters=0, model_options={}):
        ...
        self.patcher.hook_mode = totoro.hooks.EnumHookMode.MinVram
...
class VAE:
        ...
        if config is None:
            if "decoder.mid.block_1.mix_factor" in sd:
                ...
                if "blocks.2.blocks.3.stack.5.weight" in sd:
                    sd = totoro.utils.state_dict_prefix_replace(sd, {"": "decoder."})
                if "layers.4.layers.1.attn_block.attn.qkv.weight" in sd:
                    sd = totoro.utils.state_dict_prefix_replace(sd, {"": "encoder."})
                self.first_stage_model = totoro.ldm.genmo.vae.model.VideoVAE()
                ...
                self.first_stage_model = totoro.ldm.lightricks.vae.causal_video_autoencoder.VideoVAE()
    ...
    def decode_tiled_3d(self, samples, tile_t=999, tile_x=32, tile_y=32, overlap=(1, 8, 8)):
        decode_fn = lambda a: self.first_stage_model.decode(a.to(self.vae_dtype).to(self.device)).float()
        return self.process_output(totoro.utils.tiled_scale_multidim(samples, decode_fn, tile=(tile_t, tile_x, tile_y), overlap=overlap, upscale_amount=self.upscale_ratio, out_channels=self.output_channels, output_device=self.output_device))
...
def load_style_model(ckpt_path):
    ...
    if "style_embedding" in keys:
        ...
        model = totoro.ldm.flux.redux.ReduxImageEncoder()
    else:
...
def t5xxl_detect(clip_data):
    ...
    for sd in clip_data:
        if weight_name in sd:
            return totoro.text_encoders.sd3_clip.t5_xxl_detect(sd)
...
def llama_detect(clip_data):
    ...
    for sd in clip_data:
        if weight_name in sd:
            return totoro.text_encoders.hunyuan_video.llama_detect(sd)
    ...
    if len(clip_data) == 1:
        ...
        if te_model == TEModel.CLIP_G:
            ...
            if clip_type == CLIPType.SD3:
                clip_target.clip = totoro.text_encoders.sd3_clip.sd3_clip(clip_l=False, clip_g=False, t5=True, **t5xxl_detect(clip_data))
                clip_target.tokenizer = totoro.text_encoders.sd3_clip.SD3Tokenizer
            elif clip_type == CLIPType.LTXV:
                clip_target.clip = totoro.text_encoders.lt.ltxv_te(**t5xxl_detect(clip_data))
                clip_target.tokenizer = totoro.text_encoders.lt.LTXVT5Tokenizer
            else: #CLIPType.MOCHI
                clip_target.clip = totoro.text_encoders.genmo.mochi_te(**t5xxl_detect(clip_data))
                clip_target.tokenizer = totoro.text_encoders.genmo.MochiT5Tokenizer
        ...
        if clip_type == CLIPType.SD3:
            ...
            clip_target.clip = totoro.text_encoders.sd3_clip.sd3_clip(clip_l=TEModel.CLIP_L in te_models, clip_g=TEModel.CLIP_G in te_models, t5=TEModel.T5_XXL in te_models, **t5xxl_detect(clip_data))
            clip_target.tokenizer = totoro.text_encoders.sd3_clip.SD3Tokenizer
            ...
            clip_target.clip = totoro.text_encoders.flux.flux_clip(**t5xxl_detect(clip_data))
            clip_target.tokenizer = totoro.text_encoders.flux.FluxTokenizer
        elif clip_type == CLIPType.HUNYUAN_VIDEO:
            clip_target.clip = totoro.text_encoders.hunyuan_video.hunyuan_video_clip(**llama_detect(clip_data))
            clip_target.tokenizer = totoro.text_encoders.hunyuan_video.HunyuanVideoTokenizer
        else:
            clip_target.clip = sdxl_clip.SDXLClipModel
            clip_target.tokenizer = sdxl_clip.SDXLTokenizer
    elif len(clip_data) == 3:
        clip_target.clip = totoro.text_encoders.sd3_clip.sd3_clip(**t5xxl_detect(clip_data))
        clip_target.tokenizer = totoro.text_encoders.sd3_clip.SD3Tokenizer
