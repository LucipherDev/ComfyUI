...
import totoro.ldm.modules.attention
from totoro.ldm.genmo.joint_model.layers import RMSNorm
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None, pe=None):
        ...
        if mask is None:
            out = totoro.ldm.modules.attention.optimized_attention(q, k, v, self.heads, attn_precision=self.attn_precision)
        else:
            out = totoro.ldm.modules.attention.optimized_attention_masked(q, k, v, self.heads, mask, attn_precision=self.attn_precision)
...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, context=None, attention_mask=None, timestep=None, pe=None):
        ...
        x += self.attn1(totoro.ldm.common_dit.rms_norm(x) * (1 + scale_msa) + shift_msa, pe=pe) * gate_msa

        x += self.attn2(x, context=context, mask=attention_mask)

        y = totoro.ldm.common_dit.rms_norm(x) * (1 + scale_mlp) + shift_mlp
