...
if TYPE_CHECKING:
    ...
    from totoro.model_base import BaseModel
    from totoro.controlnet import ControlBase
import torch
import collections
...
def finalize_default_conds(model: 'BaseModel', hooked_to_run: dict[totoro.hooks.HookGroup,list[tuple[tuple,int]]], default_conds: list[list[dict]], x_in, timestep):
    ...
    for i, mult in enumerate(default_mults):
        ...
        for x in cond:
            ...
            p = totoro.samplers.get_area_and_mult(x, x_in, timestep)
            if p is None:
                continue
...
def calc_cond_batch(model: 'BaseModel', conds: list[list[dict]], x_in: torch.Tensor, timestep, model_options):
    executor = totoro.patcher_extension.WrapperExecutor.new_executor(
        _calc_cond_batch,
        totoro.patcher_extension.get_all_wrappers(totoro.patcher_extension.WrappersMP.CALC_COND_BATCH, model_options, is_model_options=True)
    )
...
def _calc_cond_batch(model: 'BaseModel', conds: list[list[dict]], x_in: torch.Tensor, timestep, model_options):
    ...
    hooked_to_run: dict[totoro.hooks.HookGroup,list[tuple[tuple,int]]] = {}
    default_conds = []
    ...
    for hooks, to_run in hooked_to_run.items():
        while len(to_run) > 0:
            ...
            if 'transformer_options' in model_options:
                transformer_options = totoro.patcher_extension.merge_nested_dicts(transformer_options,
...
def preprocess_conds_hooks(conds: dict[str, list[dict[str]]]):
    ...
    hook_replacement: dict[tuple[ControlBase, totoro.hooks.HookGroup], list[dict]] = {}
    for k in conds:
        for kk in conds[k]:
            if 'control' in kk:
                ...
                if len(extra_hooks) > 0:
                    hooks: totoro.hooks.HookGroup = kk.get('hooks', None)
    ...
    for key, conds_to_modify in hook_replacement.items():
        control = key[0]
        hooks = key[1]
        hooks = totoro.hooks.HookGroup.combine_all_hooks(control.get_extra_hooks() + [hooks])
...
class CFGGuider:
    ...
    def inner_sample(self, noise, latent_image, device, sampler, sigmas, denoise_mask, callback, disable_pbar, seed):
        ...
        extra_args = {"model_options": totoro.model_patcher.create_model_options_clone(self.model_options), "seed": seed}

        executor = totoro.patcher_extension.WrapperExecutor.new_class_executor(
            sampler.sample,
            sampler,
            totoro.patcher_extension.get_all_wrappers(totoro.patcher_extension.WrappersMP.SAMPLER_SAMPLE, extra_args["model_options"], is_model_options=True)
        )
    ...
    def sample(self, noise, latent_image, sampler, sigmas, denoise_mask=None, callback=None, disable_pbar=False, seed=None):
        ...
        try:
            ...
            self.model_options = totoro.model_patcher.create_model_options_clone(self.model_options)
            # if one hook type (or just None), then don't bother caching weights for hooks (will never change after first step)
            orig_hook_mode = self.model_patcher.hook_mode
            if get_total_hook_groups_in_conds(self.conds) <= 1:
                self.model_patcher.hook_mode = totoro.hooks.EnumHookMode.MinVram
            totoro.sampler_helpers.prepare_model_patcher(self.model_patcher, self.conds, self.model_options)
            executor = totoro.patcher_extension.WrapperExecutor.new_class_executor(
                self.outer_sample,
                self,
                totoro.patcher_extension.get_all_wrappers(totoro.patcher_extension.WrappersMP.OUTER_SAMPLE, self.model_options, is_model_options=True)
            )
